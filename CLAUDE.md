...
- **Sideloading for Android Auto testing**: install with `adb install app-release.apk`. To have a sideloaded (non-Play-Store) build show up on a real Android Auto head unit, enable Android Auto **Developer settings** on the phone (tap the version ~10× in Android Auto settings) and turn on **"Unknown sources"** — otherwise the phone screens work but the projected car UI won't appear.
